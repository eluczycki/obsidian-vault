...
MVP
For the mmmVP  let's start with the Core Framework and the Public Dashboard. We'll have to figure out how to get or cultivate a list of the top 25. Let's focus on the Widget layout with Velocity as the primary. We'll add the CVE and go ahead and model in the Risk. We'll have to come up with a calculation for how we're going to account for that, but we'll figure the CVE will be after MVP. We'll do that next, along with some of the other stuff.
